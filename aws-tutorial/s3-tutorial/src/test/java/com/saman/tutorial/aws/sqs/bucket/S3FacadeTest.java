...
import com.saman.tutorial.aws.sqs.bucket.utils.IoUtils;
...
import static com.saman.tutorial.aws.sqs.bucket.TestServiceFactory.createS3Facade;
...
class S3FacadeTest {
    ...
    @BeforeAll
    static void setUp() {
        underTest = Try.of(() -> createS3Facade(new S3ClientFactory(TestEnv.loadAccountProperties())))
    ...
    void createBucket_GivenBucketNameAsParam_WhenSendCreateRequest_ThenItShouldBeWaitUntilGetOKStatus(String name) {
        ...
        response.ifPresent(it -> {
            ...
            Try.of(() -> System.getProperty("vendor"))
                    .onSuccess(s -> {
                        if (s.startsWith("Aws")) {
                    ...
                    .onSuccess(s -> {
                        if (s.startsWith("Aws")) {
                            response.sdkHttpResponse()
                                    .statusText()
                                    .ifPresent(status -> assertEquals("OK", status));
                        }
                    });
            ...
            Try.of(() -> System.getProperty("vendor"))
                    .onSuccess(s -> {
                        if (s.startsWith("Aws")) {
                            it.sdkHttpResponse()
                                    .statusText()
                                    .ifPresent(status -> assertEquals("No Content", status));
                        }
                    });
        });
    }
